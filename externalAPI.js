...
async function getDetails(data) {
  ...
  data.forEach((id) => {
    userRequests.push(
      getData(
        `https://api.themoviedb.org/3/tv/${id}?api_key=${proc.envTMDB}&language=en-US&append_to_response=credits`
      )
    );
  });
...
async function getTrending() {
  const resp = axios.get(
    `https://api.themoviedb.org/3/trending/tv/day?api_key=${proc.envTMDB}`
  );
  return resp;
}
...
async function getRecommended(id) {
  const resp = axios.get(
    `https://api.themoviedb.org/3/tv/${id}/recommendations?api_key=${proc.envTMDB}&language=en-US&page=1&append_to_response=credits`
  );
  return resp;
}
